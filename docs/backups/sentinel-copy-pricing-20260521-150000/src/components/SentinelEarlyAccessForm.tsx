...
export default function SentinelEarlyAccessForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setState({
      status: "success",
      message: connected
        ? "Demande transmise à AlexCore. Elle sera relue avant activation manuelle."
        : "Demande préparée. AlexCore n'est pas encore configuré ou joignable, l'email de secours reste disponible.",
    ...
    <form onSubmit={handleSubmit} className="grid gap-4">
      ...
      <label className="flex gap-3 rounded-xl border border-white/10 bg-white/6 p-4 text-sm leading-6 text-white/72">
        ...
        <span>
          ...
          1 mois inclus, activation manuelle.
        </span>
      </label>
      <button
        type="submit"
        disabled={state.status === "loading"}
        className="rounded-full bg-white px-6 py-3 text-center font-semibold text-[#061A35] transition hover:bg-[#F8FAFC] disabled:cursor-wait disabled:opacity-70"
      >
        {state.status === "loading" ? "Envoi vers AlexCore..." : "Envoyer la demande"}
      </button>
